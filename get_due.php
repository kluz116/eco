...
$sql==""?$sql.="where c.cid in (".implode(",", $overDue).")" : $sql.=" and c.cid in (".implode(",", $overDue).")";
...
try{  
   ...
   while ($row= $date->fetch(PDO::FETCH_ASSOC)) {
	...
	$dat->execute();
    $rows = $dat->fetch(PDO::FETCH_ASSOC);
    if($rows){
  $query= "update customer_status set status='due' where cid='".$cid."'";
	$sth = $dbh->prepare($query);
	$res= $sth->execute();
	if ($res) {
		echo "client ".$lastname." Now Is Due \n";
	}else{
      ...
      if ($ras) {
        //$lastId = $dbh->lastInsertId();
          $querys= "update customer_status set status='due' where cid='".$cid."'";
          $sthh = $dbh->prepare($querys);
          $ress= $sthh->execute();
        echo "Saving clients In Overdue :".$lastname."\n";
      }
	}
