...
<?php
 ...
 if(trim($cust_id) == "")
 {
	 ...
	 if (mysql_num_rows($query_one)==0) {
	 	...
	 	if ($pay_one){
	 		$api->send_sms('VP UG Ltd',$phone,'Dear customer, we have received your payment of UGX '.$_REQUEST['amount'].'.Thank you for choosing Village Power.');	
	 	}
	 	}	 
 } 
 else
 {
	 ...
	 if($order_id != "")
	 {
		 ...
		 if($api->get_record("payment","count(*)","where `order` = '".$order_id."' /* and amt <= '".preg_replace("/[^0-9]/","",$_REQUEST['amount'])."' */ and `status` = 'pending'") == 0)
		 {
            ...
            $productvp=$api->get_record("product", "product_code", "where pid = '".$api->get_record("order_item", "item", "where `order` = '".$order_id."'")."'");
			 ...
			 if($api->get_record("payment","count(*)","where `order` = '".$order_id."' and status = 'processed'") == 1){
			 	 $down_payment=mysql_query("update `order` set order_status = 'approved' where id = '".$order_id."'");
	...
	if (mysql_num_rows($query_three)==0) {
